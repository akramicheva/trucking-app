...
export const OrdersList: React.FC = () => {
    ...
    const loadOrders = async () => {
        try {
            ...
            setOrders(data);
        } catch (error) { /* Ошибка в интерцепторе */ }
        finally { setIsLoading(false); }
    };

  if (isLoading) return <div className="text-center p-10 font-bold text-gray-500">Загрузка...</div>;
          ...
          >
            ...
            <Link 
                ...
                className="
                    ...
                    w-full py-3 mt-6
                    /* Начальное состояние: полупрозрачная и чуть уменьшенная */
                    bg-gray-50 text-gray-400 border-2 border-transparent rounded-xl
                    opacity-70 scale-[0.98]
                    transition-all duration-500 ease-out
                    
                    /* Состояние при наведении на КАРТОЧКУ (group-hover) */
                    group-hover:opacity-100 group-hover:scale-100
                    group-hover:bg-blue-600 group-hover:text-white group-hover:border-blue-600 group-hover:shadow-lg
                "
                >
